...
const AdminDashboard = () => {
	...
	const fetchIssues = async () => {
		try {
			const response = await fetch("https://jamuna-sense-backend.vercel.app/api/reports");
			const data = await response.json();
			const info = data.reports;
			// console.log(info);
			setIssues(info);
			calculateStats(info);
		} catch (err) {
	...
	const updateIssueStatus = async (issueId, newStatus) => {
		...
		const updatedIssues = issues.map((issue) => (issue.id === issueId ? { ...issue, resolved: newStatus } : issue));
		setIssues(updatedIssues);
		calculateStats(updatedIssues);
		setSelectedIssue((prev) => (prev?.id === issueId ? { ...prev, resolved: newStatus } : prev));
	};
				...
				<div className="header-content">
					...
					<div className="header-left">
						...
						<div className="title-wrap">
							<h1 className="dashboard-title">Jamuna Sense (Admin Access)</h1>
							{/* <p className="dashboard-subtitle">Real-time citizen-powered environmental dashboard</p> */}
						</div>
					</div>
							...
							>
								<div className="issue-main">
									...
									<p className="issue-location">
										<MapPin size={14} />
										{issue.location.split(" ")[0] + " " + issue.location.split(" ")[1]}
									</p>
									<div className={`status-badge status-${issue.resolved ? "resolved" : "pending"}`}>
					...
					{filteredIssues.length > 0 && totalPages > 1 && (
						<div className="pagination">
							...
							<div className="pagination-info">
								...
								<span className="pagination-count">({filteredIssues.length} total reports)</span>
							</div>
				...
				{selectedIssue && (
					<div className="detail-panel">
						<div className="detail-header">
							...
							<button onClick={() => setSelectedIssue(null)} className="close-button">
								✕
							</button>
						</div>

						<div className="detail-content">
							<img src={selectedIssue.multimedia} alt={selectedIssue.issueType} className="detail-image" />
							...
							<div className="detail-section">
								...
								<p className="detail-value">{selectedIssue.description || "No description provided"}</p>
							</div>

							<div className="detail-section">
								<label className="detail-label">Location</label>
								<p className="detail-value">{selectedIssue.reportedBy}</p>
							</div>

							<div className="detail-section">
								<label className="detail-label">Coordinates</label>
								<p className="detail-value">
									Lat: {selectedIssue.location.split(" ")[0]}, Long: {selectedIssue.location.split(" ")[1]}
								</p>
								<a
									href={`https://www.google.com/maps?q=${selectedIssue.location.split(" ")[0]},${selectedIssue.location.split(" ")[1]}`}
									target="_blank"
							...
							<div className="detail-section">
								...
								<p className="detail-value">{new Date(selectedIssue.createdAt).toLocaleString()}</p>
							</div>
							...
							<div className="detail-section">
								...
								<div className="status-action-buttons">
									<button
										onClick={() => updateIssueStatus(selectedIssue._id, false)}
										className={`status-action-btn btn-pending ${selectedIssue.resolved === false ? true : ""}`}
										disabled={selectedIssue.resolved === false}
									>
										<Clock size={18} />
										Mark as Pending
									</button>
									<button
										onClick={() => updateIssueStatus(selectedIssue._id, true)}
										className={`status-action-btn btn-resolved ${selectedIssue.resolved === true ? false : ""}`}
										disabled={selectedIssue.resolved === true}
									>
										<CheckCircle size={18} />
										Mark as Resolved
									</button>
